      ...
      <div v-for="karat in karats" :key="karat" class="karat-calculator">
        <div class="karat-header">
          ...
          <p class="rate-display">
            Rate: <strong>{{ karatRates[karat] }}₱/gram</strong>
          </p>
        </div>
        ...
        <div v-if="karatData[karat].calculated" class="result">
          <p>
            Subtotal (grams × rate + make):
            <strong>{{ karatData[karat].subtotalFormatted }}</strong>
          </p>
          <p>
            Tax (12%):
            <strong>{{ karatData[karat].taxFormatted }}</strong>
          </p>
          <p>
            Total:
            <strong>{{ karatData[karat].totalFormatted }}</strong>
          </p>
        </div>
      </div>
    </div>
    ...
    karats.forEach((k) => {
      karatData.value[k] = {
        ...
        subtotalFormatted: computed(
          () => '₱' + (karatData.value[k].subtotal ?? 0).toFixed(2)
        ),
        taxFormatted: computed(() => '₱' + (karatData.value[k].tax ?? 0).toFixed(2)),
        totalFormatted: computed(() => '₱' + (karatData.value[k].total ?? 0).toFixed(2))
      }
    })
